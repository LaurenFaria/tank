...
# Create the fish
fish = Fish("assets/sprites/red_fish.png")
all_sprites = pygame.sprite.Group()
all_sprites.add(fish)

# Create different kinds of fish (adjust the number based on your preference)
fish_images = [
    "assets/sprites/orange_fish1.png",
    "assets/sprites/green_fish.png",
    "assets/sprites/blue_fish.png",
]
all_fish_sprites = pygame.sprite.Group()

for image_path in fish_images:
    fish = Fishes(image_path)
    all_sprites.add(fish)
    all_fish_sprites.add(fish)
...
while running:
    ...
    hits = pygame.sprite.spritecollide(fish, all_fish_sprites, True)
    for hit in hits:
        if isinstance(hit, Fishes):
            fish_value += 10

    if fish_value >= required_value:
        all_fish_sprites.empty()
        new_fish = Fishes("assets/sprites/blue_fish.png")
        all_sprites.add(new_fish)
        all_fish_sprites.add(new_fish)
        fish_value = 0
